...
Our main point is about calling of a particular constructor when such TLS variable would be first accessed, in this case this would be `generate_next_tid`, which would then be stored in a `thread-local` variable `tid`. We modify this to instead generate the underlying `Pid`, so that it would be same as the `python thread` responsible for initiating the `allocation` call in the first place. (We always assume python code is the real initiator of a chain of allocation calls, and since python code would be executed inside a *running* python thread.. we could map all allocations to a python thread !).

Given that we can know `map` each allocation to a particular python thread, we could now come up with way to `suspend` and `resume` allocations for a given thread. This would give us much more control than default `with memray.Tracker` context protocol. Code could look like this:
